...
def Calculate_Full_Error(fit_errors, fit_error_vars, prior_errors, prior_error_vars):
    return fit_errors[0] / fit_error_vars[0] + fit_errors[1] / fit_error_vars[1] + prior_errors[0] / prior_error_vars[
        0] + prior_errors[1] + prior_error_vars[1]
...
def Translate_RANSAC_Estimator_Sequential(train_points, query_points, max_allowed_iteration, inlier_error_threshold,
                                          number_inliers_threshold, prior_means, prior_error_vars, fit_error_vars):
    models_list = []
    ...
    for iteration in range(max_allowed_iteration):
        ...
        remaining_inliers_indexes = Get_Remaining_Inliers(train_points, query_points, sample_indexes, base_model,
        ...
        if len(remaining_inliers_indexes) > number_inliers_threshold:
            ...
            model_all_inliers = Estimate_Translation_All_Inliers(train_points[all_inliers_indexes, :],
                                                                 query_points[all_inliers_indexes, :])

            fit_errors = Calculate_Fit_Error_XY_MSE(train_points[all_inliers_indexes, :],
                                                    query_points[all_inliers_indexes, :], model_all_inliers)
...
def Translate_RANSAC_Estimator_Parallel(train_points, query_points, max_allowed_iteration, inlier_error_threshold,
                                        number_inliers_threshold, prior_means, prior_error_vars, fit_error_vars, cores):
    models_list = []
    ...
    for iteration in range(max_allowed_iteration):
        ...
        args.append((train_points, query_points, tr_samples, qu_samples, sample_indexes, inlier_error_threshold,
                     number_inliers_threshold, prior_means, \
    ...
    if best_model is None or best_inliers is None or train_points is None or len(best_inliers) == 0 or len(
            train_points) == 0:
        return None, None, 0, sys.maxsize, -1
...
def estimateTranslation(train_points, query_points, cores):
    if cores == 1:
        ...
        best_model, inliers, perc_inliers, best_error, best_iter = Translate_RANSAC_Estimator_Sequential(train_points,
                                                                                                         query_points,
                                                                                                         700, 5, 5,
                                                                                                         None, None,
                                                                                                         None)
        ...
        best_model, inliers, perc_inliers, best_error, best_iter = Translate_RANSAC_Estimator_Parallel(train_points,
                                                                                                       query_points,
                                                                                                       700, 5, 5, None,
                                                                                                       None, None,
                                                                                                       cores)
