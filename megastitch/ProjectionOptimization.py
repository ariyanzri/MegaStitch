    ...
    def __init__(self, images, pairwise_homography_dict, image_name_to_index_dict, ref_name):
        ...
        for img_name in self.pairwise_homography_dict:
            for neighbor_name in self.pairwise_homography_dict[img_name]:
                ...
                if neighbor_name in self.pairwise_homography_dict and img_name in self.pairwise_homography_dict[
                    neighbor_name]:
...
class ReprojectionMinimization:
            ...
            for img_B_name in self.pairwise_transformations[img_A_name]:
                ...
                if self.cross_validation_k == -1 or bins is None:
                    ...
                    for i, m in enumerate(matches):
                        ...
                        row = tr_c * (V[T_A_11] * p_A[0] + V[T_A_12] * p_A[1] + V[T_A_13] * 1 - (
                                    V[T_B_11] * p_B[0] + V[T_B_12] * p_B[1] + V[T_B_13] * 1))
                        A.append(row)
                        b.append(0)

                        row = tr_c * (V[T_A_21] * p_A[0] + V[T_A_22] * p_A[1] + V[T_A_23] * 1 - (
                                    V[T_B_21] * p_B[0] + V[T_B_22] * p_B[1] + V[T_B_23] * 1))
                        A.append(row)
                        b.append(0)
            ...
            for img_B_name in self.pairwise_transformations[img_A_name]:
                ...
                for i, m in enumerate(matches):
                    ...
                    for i, k in enumerate(['UL', 'UR', 'LL', 'LR']):
                        row_x += (self.phi(i, p_A[0], p_A[1]) * C[Corner_A[k][0]] - self.phi(i, p_B[0], p_B[1]) * C[
                            Corner_B[k][0]])
                        row_y += (self.phi(i, p_A[0], p_A[1]) * C[Corner_A[k][1]] - self.phi(i, p_B[0], p_B[1]) * C[
                            Corner_B[k][1]])
    ...
    def MegaStitchTranslationParameterBased(self, anchors, noisy_corners, no_anchor_use=1):
        ...
        GPS_Width = noisy_corners[self.image_reference_name]['UR']['lon'] - \
                    noisy_corners[self.image_reference_name]['UL']['lon']
        GPS_Height = noisy_corners[self.image_reference_name]['UL']['lat'] - \
                     noisy_corners[self.image_reference_name]['LL']['lat']
        ...
        for img_A_name in self.pairwise_transformations:
            ...
            for img_B_name in self.pairwise_transformations[img_A_name]:
                ...
                translation = get_translation_in_GPS_coordinate_system(translation,
                                                                       noisy_corners[self.image_reference_name],
                                                                       self.width, self.height)

                EQ_c = self.coefs[0]

                row_x = - EQ_c * C[2 * self.n_to_i_dict[img_A_name], :] + EQ_c * C[2 * self.n_to_i_dict[img_B_name], :]
                row_y = - EQ_c * C[2 * self.n_to_i_dict[img_A_name] + 1, :] + EQ_c * C[2 * self.n_to_i_dict[
                    img_B_name] + 1, :]
    ...
    def BundleAdjustmentHomography(self, initialization, m=1e-4):
        ...
        res = least_squares(self.get_residuals, H_0, jac=self.get_jacobians, bounds=(lower_bounds, upper_bounds),
                            verbose=2)
