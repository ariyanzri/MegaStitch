...
def estimate_base_transformations(pts1,pts2,tr_type):
	...
	if tr_type == Transformation.similarity:
		...
		T =	cv2.estimateRigidTransform(pts1,pts2,False)
	...
	if tr_type == Transformation.affine:
		...
		T = cv2.estimateRigidTransform(pts1,pts2,True)
...
def estimate_transformation_from_SIFT(desc1,desc2,kp1,kp2,transformation,perc_second,cores):
	...
	if type(kp1) == cv2.KeyPoint:
		src = np.float32([[kp1[m.queryIdx].pt[0],kp1[m.queryIdx].pt[1]] for m in first_matches ])
		dst = np.float32([[kp2[m.trainIdx].pt[0],kp2[m.trainIdx].pt[1]] for m in first_matches ])
	else:
		src = np.float32([[kp1[m.queryIdx][0],kp1[m.queryIdx][1]] for m in first_matches ])
		dst = np.float32([[kp2[m.trainIdx][0],kp2[m.trainIdx][1]] for m in first_matches ])
	...
	if transformation == Transformation.affine:
		...
		T = cv2.estimateRigidTransform(src, dst , True)
	...
	if transformation == Transformation.homography:
		...
		T, masked = cv2.findHomography(src, dst, maxIters = 500, confidence = 0.99, method=cv2.RANSAC)
...
def estimate_transformation_from_Inliers(inliers,desc1,desc2,kp1,kp2,transformation):
	...
	if len(src.shape) == 1 or len(dst.shape) == 1 or src.shape[0] < 4 or dst.shape[0] < 4:
...
def draw_SIFT_points_on_img(img,kp,desc):
	res = img.copy()
	res=cv2.drawKeypoints(img,kp,res,flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
...
def keypoint_from_tuple_decode(tpl):
	kp = cv2.KeyPoint(x=tpl[0][0],y=tpl[0][1],_size=tpl[1], _angle=tpl[2], _response=tpl[3], _octave=tpl[4], _class_id=tpl[5])
	return kp
...
def normalize_key_points(kp,w,h,initial):
	...
	for p in kp:
		...
		p_new = cv2.KeyPoint(x=(p.pt[0]-w/2)/(w),y=(p.pt[1]-h/2)/(h),_size=p.size, _angle=p.angle, _response=p.response, _octave=p.octave, _class_id=p.class_id)
...
def build_transformation(transformation_type,params):
	...
	if transformation_type == Transformation.similarity:
		...
		rot_mat = cv2.getRotationMatrix2D(center_rotation, rotation_angle, uniform_scale)
