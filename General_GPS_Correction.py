...
class Configurations:
	...
	def __init__(self,images_path):
		...
		self.use_homogenouse_coords = False
...
def generate_SIFT_points(args):
	...
	if len(args) == 3:
		...
		if not os.path.exists('{0}/{1}_SIFT.data'.format(path_for_gantry,name.replace('.JPG','').replace('.tif',''))):
			
			img.load_img(True)
			kp,desc = cv_util.get_SIFT_points(img.img,None,settings.max_SIFT_points)
			pickalable_kp = [cv_util.keypoint_to_tuple_encode(p) for p in kp]
			img.delete_img()

			pickle.dump((pickalable_kp,desc), open('{0}/{1}_SIFT.data'.format(path_for_gantry,name.replace('.JPG','').replace('.tif','')), "wb"))

			return pickalable_kp,desc,name

		else:
			
			(pickalable_kp,desc) = pickle.load(open('{0}/{1}_SIFT.data'.format(path_for_gantry,name.replace('.JPG','').replace('.tif','')), "rb"))
...
def transformation_estimation_helper(args):
	...
	T , matches ,perc_inliers, inliers  = cv_util.estimate_transformation_from_SIFT(img_n_desc,img_desc,img_n_kp,img_kp,transformation,percentage_next_neighbor,cores_to_use)

	return img_name,img_n_name, T, cv_util.pickle_matches(matches), perc_inliers,inliers
...
def get_neighbor_helper(args):
	...
	neighbors+=[(img_name,img_n.name,None,None) for img_n,_ in sorted_list_all_neighbors[:settings.nearest_number]]
	neighbors+=[(img_n.name,img_name,None,None) for img_n,_ in sorted_list_all_neighbors[:settings.nearest_number]]
...
class Image:
	...
	def __init__(self,name,coords):
		...
		if coords is None:
			gps = gpsphoto.getGPSData('{0}/{1}'.format(settings.images_path,name))
	...
	def get_distance(self,img):
		...
		if settings.use_gps_distance:
			
			return cv_util.get_gps_distance(self.lat,self.lon,img.lat,img.lon)
		else:
			return math.sqrt((self.lon-img.lon)**2+(self.lat-img.lat)**2+(self.alt-img.alt)**2)
	...
	def load_img(self,for_sift=False,s=None):
		...
		if s is None: 
			...
			tmp = cv2.imread('{0}/{1}'.format(settings.images_path,self.name))
			tmp = cv2.resize(tmp,(int(settings.scale*settings.image_size[1]),int(settings.scale*settings.image_size[0])))

			if settings.equalize_histogram and for_sift:
				tmp = cv_util.histogram_equalization(tmp)

			tmp = self.crop(settings.perc_crop,tmp)
			...
			tmp = cv2.imread('{0}/{1}'.format(settings.images_path,self.name))
			tmp = cv2.resize(tmp,(int(s*settings.image_size[1]),int(s*settings.image_size[0])))
	...
	def update_SIFT_points(self,pickeled_kp,desc):
		...
		if settings.normalize_key_points:
			self.load_img()
			self.kp = cv_util.normalize_key_points(self.kp,self.img.shape[1],self.img.shape[0],(self.lon,self.lat))
...
class Field:
	...
	def __init__(self,coords=None,image_names=None,sift_p=None,tr_p=None):
	...
	def select_sub_set(self,list_images):
		...
		for img in list_images:
			...
			if max_lat - lat > settings.N_perc * (max_lat-min_lat):
				continue

			if max_lon - lon > settings.E_perc * (max_lon-min_lon):
				continue
	...
	def get_all_images(self,coords,image_names):
		...
		img_path_list = os.listdir(settings.images_path)
	...
	def get_neighbors(self):
		...
		center_ortho = (np.mean(np.array([img.lat for img in self.images])),np.mean(np.array([img.lon for img in self.images])))
		ref_image = None
		min_distance_to_center = sys.maxsize

		for img in self.images:
			d = cv_util.get_gps_distance(center_ortho[0],center_ortho[1],img.lat,img.lon) 

			if d < min_distance_to_center:
				min_distance_to_center = d
				ref_image = img

		print('>>> Reference image is {0}'.format("None" if ref_image is None else ref_image.name))
		sys.stdout.flush()
		...
		processes = multiprocessing.Pool(settings.cores_to_use)
		...
		neighbors = [(self.images_dict[img_1],self.images_dict[img_2],p3,p4) for img_1,img_2,p3,p4 in neighbors]
	...
	def refine_transformations(self,transformation_dict):
		...
		for img1 in self.images:
			...
			for img2 in self.images:
				...
				if img1.name in transformation_dict and img2.name in transformation_dict[img1.name] and\
				img2.name in transformation_dict and img1.name in transformation_dict[img2.name] and\
				not (img1.name in new_transformation_dict and img2.name in new_transformation_dict[img1.name]):
					...
					inlier_matches_1_2,inlier_matches_2_1  = self.get_intersection_DMATH(inlier_matches_1_2,inlier_matches_2_1)

					T_12 , matches_12 ,perc_inliers_12, inliers_12 = cv_util.estimate_transformation_from_Inliers(inlier_matches_1_2,img2.desc,img1.desc,img2.kp,img1.kp,settings.transformation)
					T_21 , matches_21 ,perc_inliers_21, inliers_21 = cv_util.estimate_transformation_from_Inliers(inlier_matches_2_1,img1.desc,img2.desc,img1.kp,img2.kp,settings.transformation)

					if T_12 is None or T_21 is None or perc_inliers_12<settings.discard_transformation_perc_inlier or perc_inliers_21<settings.discard_transformation_perc_inlier:
						continue
					...
					new_transformation_dict[img1.name][img2.name] = (T_12,matches_1_2,round(101-100*perc_inliers_12,2),inliers_1_2)
					new_transformation_dict[img2.name][img1.name] = (T_21,matches_2_1,round(101-100*perc_inliers_21,2),inliers_2_1)

		return new_transformation_dict

	def get_matches_bins_for_cross_validation(self,matches,inliers,number_bins,size_bins):
	...
	def generate_neighbor_transformations(self):
		...
		if self.Transformation_path is not None and os.path.exists(self.Transformation_path):
		...
		for img,img_n,_,_ in self.neighbors:
			...
			args.append((img_n.desc,img.desc,nkps,kps,settings.transformation,settings.percentage_next_neighbor,c,img_n.name,img.name))


		print(">>> Encoding keypoint information is done. Total number of neighbor transformation to estimate: {0}.".format(len(args)))
		print(">>> Begining estimating transformation.")
		sys.stdout.flush()
		

		if settings.transformation == cv_util.Transformation.translation:
			results = []

			for a in args:
				results.append(transformation_estimation_helper(a))
			
			processes = MyPool(settings.cores_to_use)
		...
		for img_name,img_n_name,T,pickled_matches,perc_inliers,inliers in results:
			...
			new_neighbors.append((self.images_dict[img_name],self.images_dict[img_n_name],T,perc_inliers))

			if T is None or perc_inliers<settings.discard_transformation_perc_inlier:
				continue

			bins = self.get_matches_bins_for_cross_validation(matches,inliers,settings.number_bins,settings.size_bins)
			...
			transformation_dict[img_name][img_n_name] = (T,matches,round(101-100*perc_inliers,2),inliers,bins)
		...
		if settings.refine_transformations:
	...
	def get_absolute_transformations_after_geo_correction(self,geo_corrected_coords):
		...
		ptsr = np.float32([[R_coords['UL'][0],R_coords['UL'][1]],\
		...
		for img in self.images_dict:
			...
			pts1 = np.float32([[geo_corrected_coords[img]['UL'][0],geo_corrected_coords[img]['UL'][1]],\
				[geo_corrected_coords[img]['UR'][0],geo_corrected_coords[img]['UR'][1]],\
				[geo_corrected_coords[img]['LR'][0],geo_corrected_coords[img]['LR'][1]],\
				[geo_corrected_coords[img]['LL'][0],geo_corrected_coords[img]['LL'][1]]])

			T = cv_util.estimate_base_transformations(pts1,ptsr,settings.transformation)
	...
	def get_initial_coord_locations(self,img_ref_name,img_ref_coords,img_w,img_h):
		...
		for img in self.images:
			...
			initial_coord_dict[img.name][0] = img_ref_coords[0] - xdiff_dict[img.name] * img_w/2
			initial_coord_dict[img.name][4] = img_ref_coords[4] + ydiff_dict[img.name] * img_h/2

			initial_coord_dict[img.name][1] = img_ref_coords[1] - xdiff_dict[img.name] * img_w/2
			initial_coord_dict[img.name][5] = img_ref_coords[5] + ydiff_dict[img.name] * img_h/2

			initial_coord_dict[img.name][2] = img_ref_coords[2] - xdiff_dict[img.name] * img_w/2
			initial_coord_dict[img.name][6] = img_ref_coords[6] + ydiff_dict[img.name] * img_h/2

			initial_coord_dict[img.name][3] = img_ref_coords[3] - xdiff_dict[img.name] * img_w/2
			initial_coord_dict[img.name][7] = img_ref_coords[7] + ydiff_dict[img.name] * img_h/2

		return initial_coord_dict

	def get_coords_from_absolute_transformations(self,transformations_dict,width,height):
	...
	def geo_correct_MGRAPH(self,do_optimize=True):
		...
		mg = MGRAPH.MGRAPH(self.images,tr,self.image_name_to_index_dict,self.get_positions(),self.reference_image,settings.transformation,settings.use_ceres_MGRAPH,settings.max_no_inliers)
		...
		for img in self.images:
			...
			if settings.normalize_key_points:
				corners = {'UL':[-0.5,-0.5,1],'UR':[0.5,-0.5,1],'LR':[0.5,0.5,1],'LL':[-0.5,0.5,1]}
			else:
				corners = {'UL':[0,0,1],'UR':[x,0,1],'LR':[x,y,1],'LL':[0,y,1]}
			...
			for key in corners:
				...
				if settings.normalize_key_points:
					...
					image_coords_dict[img.name][key] = [int(w*(new_corner[0])+w/2),int(h*(new_corner[1])+h/2)]
						
				else:

					image_coords_dict[img.name][key] = [int(new_corner[0]),int(new_corner[1])]


		print('>>> MGRAPH global optimization and coordinate calculation finished successfully.')
	...
	def geo_correct_MEGASTITCH_BNDL_Gantry_specific(self,coords,anchors_dict):
		...
		MegaStitchGPS = Bundle_Adjustment.GPS_Corner_Based_Bundle_Adjustment(self.images,tr,coords,settings.transformation,x,y,GPS_c,RPJ_c,anchors_dict,settings.scale,settings.max_no_inliers,perc_drop)
		corrected_GPS_coords = MegaStitchGPS.solve()

		print('>>> MEGASTITCH global optimization with GPS priors and coordinate calculation finished successfully.')
	...
	def geo_correct_MEGASTITCH(self):
		...
		if settings.normalize_key_points:
			...
			if settings.use_homogenouse_coords and settings.transformation == cv_util.Transformation.homography:
		...
		if settings.use_homogenouse_coords and settings.transformation == cv_util.Transformation.homography:
			...
			mega_stitch = Old_MegaStitch.MegaStitch_3L1P_Homogenouse(self.images,tr,[1,2],self.reference_image.name,img_ref_coords,settings.transformation)

		else:

			mega_stitch = Old_MegaStitch.MegaStitch_3L1P(self.images,tr,[1,2],self.reference_image.name,img_ref_coords,settings.transformation)
			...
			for key in new_coords[img.name]:
				...
				if settings.normalize_key_points:
					...
					if settings.use_homogenouse_coords and settings.transformation == cv_util.Transformation.homography:
						...
						image_coords_dict[img.name][key] = [int(w*(new_coords[img.name][key][0]/new_coords[img.name][key][2])+w/2),int(h*(new_coords[img.name][key][1]/new_coords[img.name][key][2])+h/2)]

					else:

						image_coords_dict[img.name][key] = [int(w*(new_coords[img.name][key][0])+w/2),int(h*(new_coords[img.name][key][1])+h/2)]
						
				else:
					
					if settings.use_homogenouse_coords and settings.transformation == cv_util.Transformation.homography:

						image_coords_dict[img.name][key] = [int(new_coords[img.name][key][0]/new_coords[img.name][key][2]),int(new_coords[img.name][key][1]/new_coords[img.name][key][2])]

					else:

						image_coords_dict[img.name][key] = [int(new_coords[img.name][key][0]),int(new_coords[img.name][key][1])]


		print('>>> MEGASTITCH global optimization and coordinate calculation finished successfully.')
	...
	def geo_correct_MEGASTITCH_Groups(self):
		...
		mega_stitch = Old_MegaStitch.MegaStitch_Hybrid_Multi_Group_Drone(self.images,tr,[1,2],settings.transformation,x,y,settings.normalize_key_points,\
			settings.use_homogenouse_coords,settings.use_parallel_multiGroup,settings.no_cores_multiGroup,settings.number_equation_to_pick_from_unique_tuples)

		new_coords = mega_stitch.group_propagation_correction(settings.grid_w,settings.grid_h,settings.min_intersect)
		...
		print('>>> MEGASTITCH_Replicas global optimization and coordinate calculation finished successfully.')
	...
	def geo_correct_BundleAdjustment(self,init_coords=None):
		...
		bundle_adjuster = Bundle_Adjustment.Bundle_Adjustment(self.images,tr,init_coords,[1,0.1],settings.transformation,x,y,settings.normalize_key_points,self.reference_image.name,settings.max_no_inliers)
		...
		print('>>> Bundle Adjustment global optimization and coordinate calculation finished successfully.')
	...
	def geo_correct_MegaStitch_Homography(self):
		...
		mega_stitch = Old_MegaStitch.MegaStitch_Homography(self.images,tr,[1,2],self.reference_image.name,img_ref_coords,\
			x,y,settings.normalize_key_points,settings.max_no_inliers)
	...
	def calculate_H_for_sim_GCP(self,absolute_transformations, anchors_dict, sim_GCP):
		...
		for l in sim_GCP:
			...
			p = np.array([l['img_x']*settings.scale,l['img_y']*settings.scale,1])
		...
		if settings.transformation == cv_util.Transformation.affine:
			H = cv_util.get_Similarity_Affine(np.array(From_centers), np.array(To_centers))
		
		if settings.transformation == cv_util.Transformation.similarity:
			H = cv_util.get_Similarity_Affine(np.array(From_centers), np.array(To_centers))
		
		if settings.transformation == cv_util.Transformation.homography:
			H,_ = cv2.findHomography(np.array(From_centers), np.array(To_centers), maxIters = 1000, confidence = 0.99, method=cv2.RANSAC)

		return H
		
	def calculate_projection_error_sim_GCPs(self,anchors_dict,coords,absolute_transformations,pairwise_transformations,cross_validation_k,sim_GCPs):
		...
		for i in range(5):
			...
			H = self.calculate_H_for_sim_GCP(absolute_transformations,anchors_dict,sim_GCPs[i])
				...
				for img_B_name in pairwise_transformations[img_A_name]:
					...
					if cross_validation_k == -1:
						...
						for i,m in enumerate(matches):
							...
							list_errors_normalized.append(cv_util.get_gps_distance(norm_new_p_A[1],norm_new_p_A[0],norm_new_p_B[1],norm_new_p_B[0])**2)
	...
	def calculate_projection_error(self,anchors_dict,coords,absolute_transformations,pairwise_transformations,cross_validation_k,H=None,sim_GCPs=None):

		if sim_GCPs is not None:
			return self.calculate_projection_error_sim_GCPs(anchors_dict,coords,absolute_transformations,pairwise_transformations,cross_validation_k,sim_GCPs)

		list_errors = []
		list_errors_normalized = []

		min_x = np.min([[coords[c]['UL'][0],coords[c]['UR'][0],coords[c]['LL'][0],coords[c]['LR'][0]] for c in coords])
		max_x = np.max([[coords[c]['UL'][0],coords[c]['UR'][0],coords[c]['LL'][0],coords[c]['LR'][0]] for c in coords])

		min_y = np.min([[coords[c]['UL'][1],coords[c]['UR'][1],coords[c]['LL'][1],coords[c]['LR'][1]] for c in coords])
		max_y = np.max([[coords[c]['UL'][1],coords[c]['UR'][1],coords[c]['LL'][1],coords[c]['LR'][1]] for c in coords])
		...
		return math.sqrt(np.mean(list_errors)),math.sqrt(np.mean(list_errors_normalized))
	...
	def calculate_projection_error_Gantry(self,coords,pairwise_transformations,w,h):
		...
		for image_1_name in pairwise_transformations:
			for image_2_name in pairwise_transformations[image_1_name]:
				...
				GPS_width_1 = coords[image_1_name]['UR'][0] - coords[image_1_name]['UL'][0]
				GPS_height_1 = coords[image_1_name]['UL'][1] - coords[image_1_name]['LL'][1]

				GPS_width_2 = coords[image_2_name]['UR'][0] - coords[image_2_name]['UL'][0]
				GPS_height_2 = coords[image_2_name]['UL'][1] - coords[image_2_name]['LL'][1]
				...
				for i,m in enumerate(matches):
					...
					p_1 = (coords[image_1_name]['UL'][0] + kp_1[0]/w*GPS_width_1, coords[image_1_name]['UL'][1] - kp_1[1]/h*GPS_height_1)
					p_2 = (coords[image_2_name]['UL'][0] + kp_2[0]/w*GPS_width_2, coords[image_2_name]['UL'][1] - kp_2[1]/h*GPS_height_2)
	...
	def report_GCP_error_for_Gantry(self,new_coords,old_coords,anchors_dict,x,y,is_kp=True):
		
		GPS_Width = old_coords[self.reference_image.name]['UR']['lon']-old_coords[self.reference_image.name]['UL']['lon']
		GPS_Height = old_coords[self.reference_image.name]['UL']['lat']-old_coords[self.reference_image.name]['LL']['lat']
		...
		for img in anchors_dict:
			...
			A_w = img['img_x']*settings.scale
			A_h = img['img_y']*settings.scale
			...
			GPS_diffs.append(cv_util.get_gps_distance(old_lat,old_lon,new_lat,new_lon)**2)
			
		print(':: Evaluation\n\tNumber Lids: {0}\n\tGCP-based RMSE: {1}\tGCP-based STDE: {2}'.format(len(GPS_diffs),math.sqrt(np.mean(GPS_diffs)),np.std(GPS_diffs)))
		
	def GCP_error_random_base(self,transformation_list,evaluation_list,absolute_homography_dict):
		...
		if settings.transformation == cv_util.Transformation.affine:
			H = cv_util.get_Similarity_Affine(np.array(EXP_centers), np.array(GT_centers))
			H_inv = cv_util.get_Similarity_Affine(np.array(GT_centers),np.array(EXP_centers))
		...
		if settings.transformation == cv_util.Transformation.homography:
			H,_ = cv2.findHomography(np.array(EXP_centers), np.array(GT_centers), maxIters = 1000, confidence = 0.8, method=0)
			H_inv, _ = cv2.findHomography(np.array(GT_centers),np.array(EXP_centers), maxIters = 1000, confidence = 0.8, method=0)
		...
		for l in evaluation_list:
			...
			GPS_diffs.append(cv_util.get_gps_distance(GT[1],GT[0],p_final[1],p_final[0])**2)
	...
	def get_GCP_coords_all(self, absolute_transformations,list_H_inv,anchors):
		...
		for i,a in enumerate(anchors):
			...
			p_act = [a['img_x']*settings.scale,a['img_y']*settings.scale,1]
	...
	def report_GCP_error_for_Drone_Sim_GCPs(self,absolute_homography_dict,anchors_dict,sim_GCPs):
		...
		for i in range(5):
			...
			results = self.GCP_error_random_base(transformation_list,evaluation_list,absolute_homography_dict)
		...
		if len(list_RMSE) == 5:
			...
			all_GCPs = self.get_GCP_coords_all(absolute_homography_dict,list_H_inv,anchors_dict)

			return list_H[i],list_H_inv[i], avg, list_tr_gcps, all_GCPs

		elif len(list_RMSE)>0:
			avg = np.mean(list_RMSE)
			print(':: Evaluation\n\tList RMSEs: {0}\n\tAverage GCP-based RMSE: {1}'.format(list_RMSE,avg))
			print(':: ** Number of random samplings for evaluation is less than five ({0})'.format(len(list_RMSE)))
			...
			print(':: ** Number of random samplings for evaluation is zero ({0})'.format(len(list_RMSE)))
	...
	def report_GCP_error_all_GCPs(self,absolute_homography_dict,anchors_dict):
		...
		RMSE,H,H_inv = self.GCP_error_random_base(anchors_dict,anchors_dict,absolute_homography_dict)

		print(":: GCP RMSE using all GCPS: {0}".format(RMSE))

		return H, H_inv,RMSE,None,None
	
	def report_GCP_error_for_Drone(self,absolute_homography_dict,anchors_dict,sim_GCPs=None):

		if settings.AllGCPRMSE:
			return self.report_GCP_error_all_GCPs(absolute_homography_dict,anchors_dict)

		if sim_GCPs is not None:
			return self.report_GCP_error_for_Drone_Sim_GCPs(absolute_homography_dict,anchors_dict,sim_GCPs)
		...
		for a in anchors_dict:
			...
			if key not in groupped_anchors:
				groupped_anchors[key] = [{'img_name':a['img_name'],'img_x':a['img_x'],'img_y':a['img_y'],'true_lon':a['true_lon'],'true_lat':a['true_lat']}]
			else:
				groupped_anchors[key].append({'img_name':a['img_name'],'img_x':a['img_x'],'img_y':a['img_y'],'true_lon':a['true_lon'],'true_lat':a['true_lat']})
	...
	def geo_correct_MegaStitchSimilarity(self,anchors_dict):
		...
		if settings.do_cross_validation:
			...
			for i in range(settings.number_bins):

				print("========== Fold {0} ===========".format(i))

				mega_stitch = ProjectionOptimization.ReprojectionMinimization(self.images,tr,[1,2],x,y,self.reference_image.name,settings.max_no_inliers,i)
				
				new_coords,absolute_transformations,running_time = mega_stitch.MegaStitchSimilarityAffine(True)
				
				H_GCP,_,GCP_RMSE,_,_ = self.report_GCP_error_for_Drone(absolute_transformations,anchors_dict)

				RMSE,RMSE_Normalized = self.calculate_projection_error(anchors_dict,new_coords,absolute_transformations,tr,i,H_GCP)
			...
			print("-----------------------------------------------------------")
			print(':: Mean and STD of GCP RMSE: {0}, {1}'.format(np.mean(GCP_RMSE_list),np.std(GCP_RMSE_list)))
			print(':: Mean and STD of Projection RMSE: {0}, {1}'.format(np.mean(Proj_RMSE_list),np.std(Proj_RMSE_list)))
			print(':: Mean and STD of Normalized Projection RMSE: {0}, {1}'.format(np.mean(Proj_RMSE_Norm_list),np.std(Proj_RMSE_Norm_list)))
		...
		settings.max_no_inliers = 20

		mega_stitch = ProjectionOptimization.ReprojectionMinimization(self.images,tr,[1,2],x,y,self.reference_image.name,settings.max_no_inliers,-1)
				
		new_coords,absolute_transformations,running_time = mega_stitch.MegaStitchSimilarityAffine(True)

		H,H_inv,GCP_RMSE,Sim_GCPs,all_GCPs = self.report_GCP_error_for_Drone(absolute_transformations,anchors_dict)
		
		RMSE,RMSE_Normalized = self.calculate_projection_error(anchors_dict,new_coords,absolute_transformations,tr,-1,H=H)
		...
		if settings.do_cross_validation:
			return image_coords_dict,H,H_inv,absolute_transformations,\
				...
				np.mean(Time_list)],Sim_GCPs,\
				all_GCPs
		else:
			return image_coords_dict,H,H_inv,absolute_transformations,[GCP_RMSE,RMSE,RMSE_Normalized,running_time],Sim_GCPs,all_GCPs
	...
	def geo_correct_MegaStitchAffine(self,anchors_dict,sim_GCPs):
		...
		if settings.do_cross_validation:
			...
			for i in range(settings.number_bins):
				...
				new_coords,absolute_transformations,running_time = mega_stitch.MegaStitchSimilarityAffine(False)

				H_GCP,_,GCP_RMSE,_,_ = self.report_GCP_error_for_Drone(absolute_transformations,anchors_dict,sim_GCPs)
				
				RMSE,RMSE_Normalized = self.calculate_projection_error(anchors_dict,new_coords,absolute_transformations,tr,i,sim_GCPs=sim_GCPs)
		...
		H,H_inv,GCP_RMSE,_,all_GCPs = self.report_GCP_error_for_Drone(absolute_transformations,anchors_dict,sim_GCPs)
		
		RMSE,RMSE_Normalized = self.calculate_projection_error(anchors_dict,new_coords,absolute_transformations,tr,-1,sim_GCPs=sim_GCPs)
		...
		if settings.do_cross_validation:
			...
			return image_coords_dict,H,H_inv,absolute_transformations,[GCP_RMSE,RMSE,RMSE_Normalized,running_time],all_GCPs
	...
	def geo_correct_BundleAdjustment_Homography(self,anchors_dict,sim_GCPs):
		...
		if settings.do_cross_validation:
			...
			for i in range(settings.number_bins):
				...
				MST = ProjectionOptimization.Graph(self.images,tr,self.image_name_to_index_dict,self.reference_image.name)
				initializations = MST.get_absolute_homographies()

				new_coords,absolute_transformations,running_time = mega_stitch.BundleAdjustmentHomography(initializations)
		...
		RMSE,RMSE_Normalized = self.calculate_projection_error(anchors_dict,new_coords,absolute_transformations,tr,-1,H=H,sim_GCPs=sim_GCPs)
	...
	def geo_correct_MegaStitch_Affine_Bundle_Adjustment_Homography(self,anchors_dict,sim_GCPs,use_old_inliers=False):
		...
		settings.transformation = cv_util.Transformation.affine
		tr = self.generate_neighbor_transformations()

		if not use_old_inliers:
			settings.transformation = cv_util.Transformation.homography
		...
		if settings.do_cross_validation:
			...
			for i in range(settings.number_bins):
				...
				settings.transformation = cv_util.Transformation.affine
				mega_stitch_init = ProjectionOptimization.ReprojectionMinimization(self.images,tr,[1,2],x,y,self.reference_image.name,settings.max_no_inliers,i)
				_,initializations,running_time_init = mega_stitch_init.MegaStitchSimilarityAffine(False)
				
				settings.transformation = cv_util.Transformation.homography				
				mega_stitch = ProjectionOptimization.ReprojectionMinimization(self.images,tr_h,[1,2],x,y,self.reference_image.name,settings.max_no_inliers,i)
				new_coords,absolute_transformations,running_time = mega_stitch.BundleAdjustmentHomography(initializations)
				
				H_GCP,_,GCP_RMSE,_,_ = self.report_GCP_error_for_Drone(absolute_transformations,anchors_dict,sim_GCPs)
				
				RMSE,RMSE_Normalized = self.calculate_projection_error(anchors_dict,new_coords,absolute_transformations,tr_h,i,sim_GCPs=sim_GCPs)
				print(':: Projection RMSE: {0}'.format(RMSE))
				print(':: Normalized Projection RMSE: {0}'.format(RMSE_Normalized))
		...
		settings.transformation = cv_util.Transformation.affine
		mega_stitch_init = ProjectionOptimization.ReprojectionMinimization(self.images,tr,[1,2],x,y,self.reference_image.name,settings.max_no_inliers,-1)
		_,initializations,running_time_init = mega_stitch_init.MegaStitchSimilarityAffine(False)
		
		settings.transformation = cv_util.Transformation.homography				
		mega_stitch = ProjectionOptimization.ReprojectionMinimization(self.images,tr_h,[1,2],x,y,self.reference_image.name,settings.max_no_inliers,-1)
		new_coords,absolute_transformations,running_time = mega_stitch.BundleAdjustmentHomography(initializations)
		
		H,H_inv,GCP_RMSE,_,all_GCPs = self.report_GCP_error_for_Drone(absolute_transformations,anchors_dict,sim_GCPs)
		
		RMSE,RMSE_Normalized = self.calculate_projection_error(anchors_dict,new_coords,absolute_transformations,tr_h,-1,H=H,sim_GCPs=sim_GCPs)
		print(':: Projection RMSE: {0}'.format(RMSE))
		print(':: Normalized Projection RMSE: {0}'.format(RMSE_Normalized))
		...
		if settings.do_cross_validation:
			...
			return image_coords_dict,H,H_inv,absolute_transformations,[GCP_RMSE,RMSE,RMSE_Normalized,running_time+running_time_init],all_GCPs

	def geo_correct_MegaStitch_Similarity_Bundle_Adjustment_Homography(self,anchors_dict,sim_GCPs,use_old_inliers=False):
		...
		settings.transformation = cv_util.Transformation.similarity
		tr = self.generate_neighbor_transformations()
		...
		if settings.do_cross_validation:
			...
			for i in range(settings.number_bins):
				...
				settings.transformation = cv_util.Transformation.similarity
				mega_stitch_init = ProjectionOptimization.ReprojectionMinimization(self.images,tr,[1,2],x,y,self.reference_image.name,settings.max_no_inliers,i)
				_,initializations,running_time_init = mega_stitch_init.MegaStitchSimilarityAffine(True)
		...
		RMSE,RMSE_Normalized = self.calculate_projection_error(anchors_dict,new_coords,absolute_transformations,tr_h,-1,sim_GCPs=sim_GCPs)
		print(':: Projection RMSE: {0}'.format(RMSE))
		print(':: Normalized Projection RMSE: {0}'.format(RMSE_Normalized))
	...
	def geo_correct_Translation_KP_based(self,coords,anchors_dict):
		...
		mega_stitch = ProjectionOptimization.ReprojectionMinimization(self.images,tr,[1,1],x,y,self.reference_image.name,settings.max_no_inliers,settings.scale)
		
		new_coords = mega_stitch.MegaStitchTranslationKeyPointBased(anchors_dict,coords)
	...
	def geo_correct_Translation_Parameter_based(self,coords,anchors_dict):
		...
		new_coords = mega_stitch.MegaStitchTranslationParameterBased(anchors_dict,coords)
	...
	def remove_outlier_images(self,coord_dict,perc=0.05):
		...
		for img_name in coord_dict:
			coord = coord_dict[img_name]
			if coord['UL'][0]<x_q1 or coord['UL'][0]>x_q3 or\
				coord['UR'][0]<x_q1 or coord['UR'][0]>x_q3 or\
				coord['LL'][0]<x_q1 or coord['LL'][0]>x_q3 or\
				coord['LR'][0]<x_q1 or coord['LR'][0]>x_q3 or\
				coord['UL'][1]<y_q1 or coord['UL'][1]>y_q3 or\
				coord['UR'][1]<y_q1 or coord['UR'][1]>y_q3 or\
				coord['LL'][1]<y_q1 or coord['LL'][1]>y_q3 or\
				coord['LR'][1]<y_q1 or coord['LR'][1]>y_q3:
	...
	def visualize_initial_coords(self,initial_coord_dict):
		coords = np.array([[(initial_coord_dict[k][0]+initial_coord_dict[k][1])/2,(initial_coord_dict[k][5]+initial_coord_dict[k][6])/2] for k in initial_coord_dict])
	...
	def save_visualized_initial_coords(self,initial_coord_dict,save_path,img_ref_index):
		coords = np.array([[(initial_coord_dict[k][0]+initial_coord_dict[k][1])/2,(initial_coord_dict[k][5]+initial_coord_dict[k][6])/2] for k in initial_coord_dict])

		fig = plt.figure()
		ax = fig.add_subplot(111)

		ax.scatter(np.delete(coords[:,0],img_ref_index,axis=0), np.delete(coords[:,1],img_ref_index,axis=0), marker='^')
		ax.scatter(coords[img_ref_index,0], coords[img_ref_index,1], marker='^',color='red')
		...
		if is_2d:
			...
			for img,img_n,transformation,perc_inliers in self.neighbors:
				...
				if perc_inliers is None:
					...
					if perc_inliers<settings.discard_transformation_perc_inlier:
						c=(0,0,0,1)
					else:
			...
			for img,img_n,transformation,perc_inliers in self.neighbors:
				...
				ax.plot([img.lon,img_n.lon],[img.lat,img_n.lat],[img.alt,img_n.alt],color=c)
		
		plt.show()

	def save_field_centers_visualization(self,save_path):

		if settings.Dataset == "ODFN":
			return 
	...
	def convert_GPS_coords_to_image_coords(self,gps_coords):
		...
		w_GPS = gps_coords[self.reference_image.name]['UR'][0] - gps_coords[self.reference_image.name]['UL'][0]
		h_GPS = gps_coords[self.reference_image.name]['LL'][1] - gps_coords[self.reference_image.name]['UL'][1]
		...
		for img_name in gps_coords:
			...
			for k in ['UL','UR','LR','LL']:
				...
				image_coords_dict[img_name][k] = [w_ratio*(coord[k][0]-min_x),h_ratio*(coord[k][1]-min_y)]
	...
	def generate_field_ortho(self,image_coords_dict,s=None,orig_GPS=None,gcp_info=None):

		if orig_GPS is not None:

			image_coords_dict = self.convert_GPS_coords_to_image_coords(image_coords_dict)
		...
		if s is not None:
			...
			for img in image_coords_dict:
				...
				coord = {'UL':[int(old_coord['UL'][0]*c),int(old_coord['UL'][1]*c)],\
		...
		if (settings.Dataset == "GRG" and settings.Method == 'BNDL-ADJ') or settings.Dataset == "ODFN":
		...
		if settings.parallel_stitch:
			...
			for img in self.images:
				...
				pts2 = np.float32([[img_coords['UL'][0]-min_x,img_coords['UL'][1]-min_y],\
				...
				if img == self.reference_image:
					...
					if settings.draw_guided_colors:
						img.img = cv2.rectangle(img.img, (0,0), (img.img.shape[1],img.img.shape[0]), (0,0,255), 10) 
		...
		if settings.draw_guided_colors:
			...
			cv2.line(frame_image,(-min_x,-min_y),(-min_x+50,-min_y),(0,255,0),2)
			cv2.line(frame_image,(-min_x,-min_y),(-min_x,-min_y+50),(0,255,0),2)
			...
			for img in self.images:
				...
				pts2 = np.int32([[img_coords['UL'][0]-min_x,img_coords['UL'][1]-min_y],\
					[img_coords['UR'][0]-min_x,img_coords['UR'][1]-min_y],\
					[img_coords['LR'][0]-min_x,img_coords['LR'][1]-min_y],\
					[img_coords['LL'][0]-min_x,img_coords['LL'][1]-min_y]])

				cv2.line(frame_image,(pts2[0][0],pts2[0][1]),(pts2[1][0],pts2[1][1]),(255,255,0),2)
				cv2.line(frame_image,(pts2[1][0],pts2[1][1]),(pts2[2][0],pts2[2][1]),(255,255,0),2)
				cv2.line(frame_image,(pts2[2][0],pts2[2][1]),(pts2[3][0],pts2[3][1]),(255,255,0),2)
				cv2.line(frame_image,(pts2[3][0],pts2[3][1]),(pts2[0][0],pts2[0][1]),(255,255,0),2)
				
		if settings.draw_GCPs and gcp_info is not None:
			
			s1, s2 = cv_util.get_GCP_sizes(settings.Dataset,settings.Method)
			...
			for i,a in enumerate(anchors):
				...
				cv2.rectangle(frame_image,(int((p_act[0]-min_x)*C-s2-2),int((p_act[1]-min_y)*C-s2-2)),(int((p_act[0]-min_x)*C+s2+2),int((p_act[1]-min_y)*C+s2+2)),(0,0,0),-1)
				cv2.rectangle(frame_image,(int((p_act[0]-min_x)*C-s2),int((p_act[1]-min_y)*C-s2)),(int((p_act[0]-min_x)*C+s2),int((p_act[1]-min_y)*C+s2)),color2,-1)

				p = [a['img_x']*settings.scale,a['img_y']*settings.scale,1]
				p_GPS = np.matmul(absolute_transformations[a['img_name']],p)
				p_GPS = p_GPS/p_GPS[2]
				
				cv2.circle(frame_image,(int((p_GPS[0]-min_x)*C),int((p_GPS[1]-min_y)*C)),s1+4,(0,0,0),-1)
				cv2.circle(frame_image,(int((p_GPS[0]-min_x)*C),int((p_GPS[1]-min_y)*C)),s1,color1,-1)

			
		print('>>> Orthomosaic generated successfully.')
		return frame_image.astype('uint8')

	def generate_field_ortho_multiple_GCPs(self,image_coords_dict,s=None,orig_GPS=None,gcp_info=None):
		...
		if settings.Dataset == "GRG" and settings.Method == 'BNDL-ADJ':
			print(":: Removing outlier images for this experiment.")
		...
		if settings.draw_GCPs and gcp_info is not None:
			...
			if settings.Dataset == "DSEFN" or settings.Dataset == "DLLFN":
				s1 = 30
				s2 = 40
			else:
				s1 = 10
	...
	def generate_field_ortho_GPS(self,gps_coords):
		...
		pts1 = np.float32([[ref_gps['UL'][0],ref_gps['UL'][1]],\
	...
	def save_field_coordinates(self,path,corrected_coordinates):
		...
		info = {'scale':settings.scale,'coords':corrected_coordinates,'min_x':min_x,'min_y':min_y,'max_x':max_x,'max_y':max_y}
	...
	def get_gps_scale_factors(self,coords,orig_GPS):
		...
		w_GPS = orig_GPS[self.reference_image.name]['UR']['lon'] - orig_GPS[self.reference_image.name]['UL']['lon']
		h_GPS = orig_GPS[self.reference_image.name]['UL']['lat'] - orig_GPS[self.reference_image.name]['LL']['lat']
	...
	def calculate_sigma_reprojection_error(self,coords,orig_GPS=None,is_GPS=False):
		...
		absolute_transformations = self.get_absolute_transformations_after_geo_correction(corrected_coord)
		...
		print('>>> Variance reprojection errors after correction: {0}'.format(np.std(reprojection_errors)))
	...
	def generate_transformation_accuracy_histogram(self,coords,save_path):
		...
		for c in coords:
			...
			coord_center = ((coords[c]['UL'][0]+coords[c]['UR'][0]+coords[c]['LL'][0]+coords[c]['LR'][0])/4,\
				(coords[c]['UL'][1]+coords[c]['UR'][1]+coords[c]['LL'][1]+coords[c]['LR'][1])/4)
